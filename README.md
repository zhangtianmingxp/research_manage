...
> **项目状态：进行中，尚未完成。**
>
> 当前已完成29个研究论文 PDF 的文件清单、schema v2、1篇论文（P0008）的全量公共元数据试点，以及3篇论文（P0001、P0009、P0012）的项目级小批量核验。除P0008外，其余论文尚未逐篇完成SRR/ERR/DRR、ENA下载链接及样本到Run/File映射核验。本仓库当前结果不能作为最终完整数据集使用。
...
- 第4轮完成3篇项目级小批量核验：P0001（`E-MTAB-1948`/`ERP004055`）、P0009（`GSE129997`/`PRJNA533460`/`SRP192917`）、P0012（`GSE168251`/`PRJNA706679`），但未展开Run/File级下载链接。

第一轮完整执行记录见 [ans_qes/result1.md](ans_qes/result1.md)，P0008试点说明见 [reports/per_paper/P0008_pilot.md](reports/per_paper/P0008_pilot.md)，第4轮小批量摘要见 [reports/batch_round4_summary.md](reports/batch_round4_summary.md)。

### 尚未完成

- 除P0008与P0001/P0009/P0012外，其余24篇规范论文的正式题名、DOI、物种和实验设计尚未逐篇核验。
- 尚未系统下载和检查所有论文的补充材料。
- P0008之外的公共项目尚未展开完整 GSM/SRS/SRX/SRR/ERR/DRR 关系；第4轮只登记了3篇论文的项目级归档入口和代表性实验阶段。
...
| `data/curated/papers.tsv` | 28篇规范论文记录 | P0008全量试点；P0001/P0009/P0012完成项目级核验 |
| `data/interim/pilot/experiments.tsv` | 试点与小批量实验设计 | P0008全量试点加3篇round4代表性实验 |
| `data/interim/pilot/archive_samples.tsv` | 60个GSM及BioSample/SRX处置 | P0008全量公共元数据试点 |
| `data/interim/pilot/samples_timepoints.tsv` | 同步化、采样点与周期阶段 | P0008记录加round4代表性阶段 |
| `data/interim/pilot/perturbations.tsv` | 扰动技术、靶标和效果 | P0008代表性扰动加P0012 CTCF/triptolide扰动 |
| `data/interim/pilot/accessions.tsv` | GEO/BioProject/SRA/ENA层级 | P0008含1,290个真实Run；round4仅项目级 |
| `data/interim/pilot/files.tsv` | ENA API返回的FASTQ文件字段 | 2,580条，不含文件正文 |
| `data/interim/pilot/source_queries.tsv` | endpoint、参数、状态、哈希和重试 | 含P0008查询与round4轻量官方快照 |
| `data/interim/pilot/semantic_review.tsv` | 逐记录高风险语义审计 | 含P0008审计与round4未决语义边界 |
| `data/interim/pilot/evidence.tsv` | 字段级论文与数据库证据 | 含P0008试点证据与round4证据 |
| `data/interim/pilot/unresolved_issues.tsv` | 缺失、冲突和待裁决问题 | 含P0008和round4未决问题 |
...
不会自动执行新生成的 prompt、自动进入下一轮、自动 commit 或自动 push。当前第4轮执行后应停在等待结果审查状态；不会自动生成 `prompt5.md`。
...
4. 将第4轮3篇论文的项目级核验扩展到轻量Run metadata，再决定是否分批处理剩余24篇规范论文。
